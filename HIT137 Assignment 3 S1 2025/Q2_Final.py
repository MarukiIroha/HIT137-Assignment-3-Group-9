...
def setup_level(level):
    ...
    if level == 1:
        ...
        platforms = [  # <-- 這裡新增 2 個平台
            Platform(370, 160, 60, 10),
            Platform(565, 160, 85, 10),
            Platform(0, 319, 800, 100)
        ]
    elif level == 2:
        enemies = [
...
async def main():
    ...
    while True:
        ...
        # Player feet for platform detection
        player_feet = pygame.Rect(man.x + 17, man.y + man.height, man.width - 34, 5)
        on_ground = False
        for plat in platforms:
            if player_feet.colliderect(plat.rect) and vel_y >= 0:
                man.y = plat.rect.top - man.height
                vel_y = 0
                on_ground = True
                break
